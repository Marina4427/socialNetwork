...
const Requests = () => {
  ...
  const cancelRequest = async (id) => {
    try {
      ...
      const { data: targetUser } = await axios.get(`/users/${id}`);

      await axios.patch(`/users/${id}`, {
        notifications: targetUser.notifications.filter(
          (n) => n.from !== user.id || n.type !== "friend_request"
        ),
      });
    } catch (error) {
    ...
    <section className="requests">
      <div className="container">
        <div className="notification__content">
          <h2 className="notification__title"> Запросы </h2>
